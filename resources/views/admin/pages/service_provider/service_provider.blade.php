            ...
            <div class="page-content">
                <div class="container-fluid">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">{{__('admin.service_provider')}}</h4>
                        <div class="page-title-right d-flex align-items-center gap-3 flex-wrap">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="{{route('home')}}"> {{__('admin.dashboard')}} </a></li>
                                <li class="breadcrumb-item active">{{__('admin.service_provider')}}</li>
                            </ol>
                        </div>
                    </div>
                    <div class="card">
                                ...
                                <tbody>
                                    @foreach($service_providers as $user)
                                        <tr id="{{$user->id}}">
                                            ...
                                            <td>{{$user->phone}}</td>
                                            @if(admin_user()->havePermission('service_provider_edit'))
                                                <td style="display: inline-block; min-width: 100px">
                                                    <form onsubmit="return submitForm(this)" id="{{"wallet-form"}}" style="display: flex; width: 100%; flex: 1; gap: 8px;" action="{{route('service_provider.update_wallet')}}" method="POST" enctype="application/x-www-form-urlencoded">
                                                        @csrf
                                            ...
                                            <td><a href="{{route('service_orders.users',['type'=>"service_provider",'id'=>$user->id])}}" class="btn btn-outline-warning">{{__('buttons.show_service_orders')}}</a></td>
                                            @if(admin_user()->havePermission('service_provider_edit'))
                                                <td>
                                                    <button class="btnn bttn-danger" data-bs-toggle="modal" data-bs-target="#confirmModal"
                                                            data-id="{{ $user->id }}" data-url="{{ route('service_provider.delete')}}"
                                                            id="delete_dialog">
...
@section('js')
    <script>
        ...
        function submitFormButton(button) {
            ...
            var url = '{{ route("service_provider.update_block") }}';
